    ...
    >
      ...
      >
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.borderColor = C.primary;
            e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,6,102,.1)';
          }}
          onMouseLeave={(e) => {
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
              Kendi gelir-gider ve harcamalarını yönet
            </div>
          </div>
          <div style={{ marginLeft: 'auto', fontSize: 20, color: C.subtle }}>
            →
          </div>
        </button>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
              Kredi kartı, kredi ve elden borçlarını takip et
            </div>
          </div>
          <div style={{ marginLeft: 'auto', fontSize: 20, color: C.subtle }}>
            →
...
function PersonalAccounting({
  ...
  const CATS = [
    { id: 'loan', label: 'Kredi', icon: '🏦' },
    { id: 'cc', label: 'Kredi Kartı', icon: '💳' },
    { id: 'bill', label: 'Fatura', icon: '📄' },
    { id: 'personal', label: 'Kişisel Ödeme', icon: '🤝' },
  ];
  ...
  const ICON_MAP = [
    ...
    // Faturalar
    {
      keys: ['elektrik', 'tedaş', 'enerjisa', 'toroslar', 'toki enerji'],
      icon: '⚡',
    },
    { keys: ['su', 'iski', 'aski', 'buski', 'su faturası'], icon: '💧' },
    {
      keys: ['doğalgaz', 'gaz', 'igdaş', 'gazdaş', 'botaş', 'doğal gaz'],
      icon: '🔥',
    },
    // Sigorta & Aidat
    {
      keys: ['sigorta', 'dask', 'kasko', 'trafik', 'sağlık sigortası'],
      icon: '🛡️',
    },
    { keys: ['aidat', 'apartman', 'site aidat', 'yönetim'], icon: '🏢' },
    // Eğitim
    {
      keys: [
        'okul',
    ...
    // Sağlık
    {
      keys: ['doktor', 'hastane', 'klinik', 'diş', 'eczane', 'sağlık'],
      icon: '🏥',
    },
    // Ulaşım
    {
    ...
    // Kira
    { keys: ['kira', 'ev kirası', 'işyeri kirası', 'ofis kirası'], icon: '🏠' },
    // Banka & Kredi
    {
      keys: [
        'akbank',
    ...
    {
      keys: ['kredi kartı', 'kart borcu', 'kart taksit', 'taksit'],
      icon: '💳',
    },
    {
      keys: ['kredi', 'ihtiyaç kredisi', 'konut kredisi', 'taşıt kredisi'],
      icon: '💰',
    },
    // Abonelik
    {
    ...
    // Yemek
    { keys: ['yemek', 'restoran', 'kafe', 'kahve', 'öğle yemeği'], icon: '🍽️' },
    // Spor
    { keys: ['spor', 'gym', 'fitness', 'yüzme', 'spor salonu'], icon: '💪' },
    // Vergi & Devlet
    {
      keys: [
        'vergi',
    ...
    // Çocuk
    { keys: ['kreş', 'anaokul', 'bebek', 'çocuk'], icon: '👶' },
  ];
  ...
  const setF = (k, v) => {
    setForm((p) => {
      const next = { ...p, [k]: v };
      // İsim yazılınca otomatik ikon eşleştir (kullanıcı değiştirmediyse)
      if (k === 'name') {
        const auto = getIcon(v);
        if (auto && !p._iconManual) next.icon = auto;
        if (!v) next.icon = '';
      }
      if (k === 'icon') next._iconManual = true;
      return next;
    });
    setFormErr((p) => ({ ...p, [k]: '' }));
  };
  ...
  const addItem = async () => {
    ...
    if (!form.dueDate) e.dueDate = 'Tarih girin';
    if (Object.keys(e).length) {
      setFormErr(e);
      return;
    }
    try {
      ...
      setFormErr({ name: err.message });
    }
  };

  const payItem = async (id) => {
    setPaidIds((p) => [...p, id]);
    try {
      await onPay(id);
    } catch (err) {
    ...
    setTimeout(() => setPaidIds((p) => p.filter((x) => x !== id)), 400);
  };
  ...
  const dayD = (s) =>
    s ? Math.ceil((new Date(s) - new Date()) / 864e5) : null;
  const dueLbl = (s) => {
    const x = dayD(s);
    if (x === null) return '';
    if (x < 0) return Math.abs(x) + 'g gecikmiş';
  ...
  return (
    ...
    >
        ...
        {!loading && items.length === 0 && (
          ...
          >
            ...
            <div style={{ fontSize: 12 }}>
              Sağ üstteki "Ödeme Ekle" butonuyla başla
            </div>
          </div>
        )}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {unpaid.map((item) => {
                ...
                const animating = paidIds.includes(item.id);
                return (
                  <div
                    key={item.id}
                    style={{
      ...
      {/* Ödeme ekleme formu — bottom sheet */}
      {showForm && (
        <>
          <div
          ...
          >
            ...
            <div>
              ...
              >
                {CATS.map((cat) => {
                  const active = form.cat === cat.id;
                  return (
                    <button
                      key={cat.id}
                      onClick={() => setF('cat', cat.id)}
                      style={{
                        display: 'flex',
                        flexDirection: 'column',
                        alignItems: 'center',
            ...
            <div>
              ...
              <div
                style={{
                  background: formErr.name ? '#fef2f2' : '#f9fafb',
                  border: `1.5px solid ${formErr.name ? '#dc2626' : 'transparent'}`,
                  borderRadius: 9,
                  padding: '9px 12px',
                }}
                onFocusCapture={(e) =>
                ...
                onBlurCapture={(e) =>
                  (e.currentTarget.style.borderColor = formErr.name
                    ? '#dc2626'
                    : 'transparent')
                }
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <input
                    ...
                    placeholder="ör. Akbank taksit, DASK, kira..."
                    style={{
                      flex: 1,
                      background: 'transparent',
                      border: 'none',
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {[
                  ...
                  <button
                    key={ico}
                    onClick={() => setF('icon', ico)}
                    style={{
                      width: 36,
                      height: 36,
                      borderRadius: 8,
                      background: form.icon === ico ? '#eff6ff' : '#f3f4f6',
                      border: `1.5px solid ${form.icon === ico ? C.primary : 'transparent'}`,
                      cursor: 'pointer',
                      fontSize: 18,
                      display: 'flex',
                      alignItems: 'center',
            ...
            >
              <div>
                ...
                <div
                  style={{
                    background: formErr.amount ? '#fef2f2' : '#f9fafb',
                    border: `1.5px solid ${formErr.amount ? '#dc2626' : 'transparent'}`,
                    borderRadius: 9,
                    padding: '9px 12px',
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  onBlurCapture={(e) =>
                    (e.currentTarget.style.borderColor = formErr.amount
                      ? '#dc2626'
                      : 'transparent')
                  }
                >
                  <span
                    style={{ fontWeight: 700, color: C.subtle, fontSize: 13 }}
                  >
                    ₺
                  </span>
                  <input
              ...
              <div>
                ...
                <div
                  style={{
                    background: formErr.dueDate ? '#fef2f2' : '#f9fafb',
                    border: `1.5px solid ${formErr.dueDate ? '#dc2626' : 'transparent'}`,
                    borderRadius: 9,
                    padding: '9px 12px',
                  }}
                  onFocusCapture={(e) =>
                  ...
                  onBlurCapture={(e) =>
                    (e.currentTarget.style.borderColor = formErr.dueDate
                      ? '#dc2626'
                      : 'transparent')
                  }
                >
            ...
            <Btn onClick={addItem} full size="lg">
              ✓ Ekle
            </Btn>
          </div>
        </>
...
export default function App() {
  ...
  if (appMode === null)
    return (
      <ModeSelectScreen onSelect={setAppMode} user={user} onLogout={logout} />
    );
  ...
  if (appMode === 'personal')
    return (
      <PersonalAccounting
        ...
        isMobile={isMobile}
        items={personalPayments}
        loading={ppLoading}
        onAdd={addPersonalPayment}
        onPay={payPersonalPayment}
        onDelete={deletePersonalPayment}
      />
    );
